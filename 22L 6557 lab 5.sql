...
select * from tweets
where tweetid =any(select tweetid from likes
where likebyusername  =any(select followerusername from following
where followedusername = 'Imrankhan'))

select distinct USERNAME from UserInterests 
where InterestID =(select distinct InterestID from Interests where description = 'politics')

select likebyusername from likes
where tweetid in(select TweetID from tweets where username='DonaldTrump')

select* from tweets where TweetID in
(select TweetID from Likes where username in
(select username from Interests where InterestID in(select InterestID from Interests where Description ='Politics' or Description ='Sports')))

select username from users where username not in
(select distinct followerusername from Following)

select likebyusername from likes where tweetid in
(select TweetID from Tweets where text like '%#MasseCritique&#39%')
...
(select UserName from UserInterests where InterestID in(select InterestID from Interests where Description ='sports' and Description ='movies'))

select * from tweets where TweetID in(select TweetID from Likes where LikeByUserName in
(select followerusername from Following where FollowedUserName ='Ali123'))
...
select* from tweets where TweetID in
(select TweetID from likes where LikeByUserName in
(select username from UserInterests where InterestID in(select InterestID from Interests where Description ='education')))